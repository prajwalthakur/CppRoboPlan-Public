...
void DifferentialIk::init()
{
    ...
    rplCollection<rplUnSignedInt> seedVector = cpproboplan::generateRandomSeed(mIkOptions.rng_seed, mPinModel.nv);
...
bool DifferentialIk::solve(const std::string& targetFrame, 
                    ...
                    const std::optional<rplState>& initSolutionJoint )
{
    init();
    

    // get the target frame id 
    rplUnSignedInt targetFrameId = mPinModel.getFrameId(targetFrame);
    rplUnSignedInt numActiveJoints = mPinModel.nv;
    ...
    // If the guess Solution is not provided, we will generate one from  the random vector generator
    rplState guessJointVector;
    if(!initSolutionJoint)
    {
        


        std::pair<rplState,bool> res =  crGetRandomCollisionFreeSample(mPinModel, 
                                                            mData, 
                                                            mCollisionModel, 
    ...
    {   
        // also remove the ignored_joint_indices , in this case last 2 (gripper Joints)
    ...
    rplState currJointVector = guessJointVector; 
    ...
    {   
        ...
        {
            // Generate a New random Sample, for initial guess solution for Ik algorithm.
            std::pair<rplState,bool> res =  crGetRandomCollisionFreeSample(mPinModel, 
                                                                mData, 
                                                                mCollisionModel, 
                                                                mCollisionData, mIkOptions.collision_safety_margin, mIkOptions.max_tries, mRandomVecGenerator);
            if(!res.second)
            {
                std::cerr << " could not  able to generate the collision free Joint Sample"<<std::endl;
                break;
            }
...
bool DifferentialIk::checkTeminationCondition( rplState& guessJointVector)
{
...
rplState DifferentialIk::getResult()
{
